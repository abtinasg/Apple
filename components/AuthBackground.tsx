export default function AuthBackground({ children }: { children: React.ReactNode }) {
  return (
    <main className="relative min-h-screen flex items-center justify-center overflow-hidden py-10">
      <div
        className="absolute inset-0"
        style={{ background: 'radial-gradient(ellipse at 50% -10%, #1a1a3e 0%, #0d0d1a 40%, #0a0a0a 70%)' }}
      />
      <div
      ...
      />
      <div className="relative z-10 w-full max-w-[400px] px-4">{children}</div>
    </main>
  )
}
